...
| `GET` | `/zaplab/api/config` | Read current application configuration (`recover_edits`, `recover_deletes`, `suppress_delivery_receipts`, `appear_offline`, …) |
| `PUT` | `/zaplab/api/config` | Update configuration fields — partial update, only supplied fields are changed |
| `GET` | `/zaplab/api/events/recent` | Last N events from DB (query params: `limit` 1–500, `type` filter) |
| `GET` | `/zaplab/api/events/stream` | SSE stream of live WhatsApp events (query params: `token`, `type` filter) |
...
- **Topbar Quick Toggles** — One-click toggles directly in the topbar: **Appear Offline** (wifi icon, green=online / red=offline) and **Suppress Delivery Receipts** (check-circle icon, shows strikethrough when suppressed).
...
| **Live Events** | Real-time event stream via SSE — filterable by type, syntax-highlighted JSON, resizable panel; connection status indicator (connecting/connected/disconnected dot); **Reconnect** button when disconnected; **Reload** button to re-fetch the last 100 events from history |
...
| **App State Inspector** | Read-only inspection of the three whatsmeow app state SQLite tables; **Collections tab** — all known collections (`critical`, `regular`, etc.) with version index and Merkle-tree state hash; **Sync Keys tab** — app state decryption key metadata (key ID, timestamp, fingerprint, size); **Mutations tab** — per-leaf HMAC integrity codes (index MAC + value MAC) for any selected collection |
| **Multi-Session Comparator** | Select up to 6 Signal Protocol sessions or group SenderKey records and compare them side by side; differences relative to the first selected are highlighted in amber; diff count badge; covers version, chain counters, identity keys, raw size, and decode error |
| **Network Graph** | Interactive force-directed graph of the WhatsApp contact/group network derived from stored Message events; node types: self (blue), contacts (orange), groups (green), broadcast (purple); edge thickness = message count; node radius scales with message volume; push-name label enrichment from `whatsmeow_contacts`; click a node to inspect connections; drag to reposition; Pause/Resume simulation; period selector |
...
| **Auto-Reply Rules** | Automated response engine: evaluate incoming messages against a priority-ordered list of rules; conditions include sender direction (others/me/all), specific chat/sender JID, text pattern (prefix/contains/exact/regex), case sensitivity, and hour window (midnight-wrap aware); actions: `reply` (quoted reply with template vars `{name}` `{sender}` `{chat}` `{text}` `{hour}`), `webhook` (async POST), `script` (run stored script); `stop_on_match` flag; per-rule hit counter and last-match timestamp; full CRUD API + toggle endpoint |
| **Scheduled Messages** | Schedule future message sends stored in the `scheduled_messages` collection; minute-tick worker fires up to 10 pending messages per cycle; status lifecycle: pending → sent / failed / cancelled; full CRUD API with ISO-8601 scheduled_at, reply-to support, and optional cancel |
| **Reaction Tracking** | Incoming encrypted reactions (`EncReactionMessage`) are decrypted and persisted to `message_reactions`; query by message ID, chat, or sender; stats endpoint returns top emojis and top reactors per chat |
| **Mention Tracker** | Every `@mention` JID in incoming messages is extracted from `ContextInfo` and persisted to `mentions`; `is_bot` flag when the bot's own JID is referenced; stats endpoint for mention counts per JID and bot-mention totals |
| **Contact Cache** | `contact_cache` collection caches name, phone, about, and avatar URL per JID; upserted asynchronously after every live `GetContactInfo()` call; API: search, refresh single contact, background populate from the full whatsmeow store, delete entry |
| **Webhook Delivery Log** | Every outgoing webhook attempt (success and failure) is persisted to `webhook_deliveries` with status, attempt count, HTTP code, and error message; HMAC-SHA256 signing via `WEBHOOK_SECRET`; exponential retry (immediate + 5 s + 25 s); API: list with status/URL filters, purge by age |
| **Prometheus Metrics** | `GET /metrics` (public) exposes connection state, event queue depth, active tracker count, events/24h by type, receipt latency p50/p95, webhook delivery stats, and scheduled message counts in Prometheus text format |
...
| **Settings** | General application configuration: toggle Message Recovery for edits and deletes; manage API token; **Suppress Delivery Receipts** toggle (also in topbar); **Appear Offline** toggle (also in topbar — persists across reconnects) |
