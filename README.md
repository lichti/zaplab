...
| **Scripting** | JavaScript automation sandbox powered by goja; persisted scripts with name, description, configurable timeout, enable/disable toggle; sandbox APIs: `console.log`, `wa.sendText(jid, text)`, `wa.status()`, `http.get(url)`, `http.post(url, body)`, `db.query(sql)`, `sleep(ms)`; ad-hoc console for quick one-off scripts; last-run status/output/duration stored per script |
