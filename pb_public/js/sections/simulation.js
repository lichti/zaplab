  ...
  return {
    ...
    async simStart() {
      ...
      try {
        ...
        const initRes = await this.zapFetch('/zaplab/api/sendelivelocation', {
          method:  'POST',
        ...
        const simRes  = await this.zapFetch('/zaplab/api/simulate/route', {
          method:  'POST',
          headers: { 'Content-Type': 'application/json', 'X-API-Token': this.apiToken },
          body: JSON.stringify({
            to:               this.sim.to,
    ...
    async simStop() {
      ...
      try {
        await this.zapFetch(`/zaplab/api/simulate/route/${this.simActive.id}`, {
          method:  'DELETE',
    ...
    simStartPolling() {
      ...
      this.simPollingInterval = setInterval(async () => {
        try {
          const res  = await this.zapFetch('/zaplab/api/simulate/route', { headers: { 'X-API-Token': this.apiToken } });
