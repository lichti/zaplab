...
func sendTimedMsg(chatID types.JID, text string) (*waE2E.Message, *whatsmeow.SendResponse, error) {
	newmsg := &waE2E.Message{
		ExtendedTextMessage: &waE2E.ExtendedTextMessage{
			Text: proto.String(text),
			ContextInfo: &waE2E.ContextInfo{
				Expiration: proto.Uint32(60),
			},
		},
	}
	return sendMessage(chatID, newmsg)
}

func removeOldMsg(chatID types.JID, msgID string) (*waE2E.Message, *whatsmeow.SendResponse, error) {
	newmsg := &waE2E.Message{
		ProtocolMessage: &waE2E.ProtocolMessage{
			...
			TimestampMS: proto.Int64(time.Now().Unix() * 1000),
...
func editOldMsg(chatID types.JID, msgID string, newMsg string) (*waE2E.Message, *whatsmeow.SendResponse, error) {
	newmsg := &waE2E.Message{
		ProtocolMessage: &waE2E.ProtocolMessage{
			Type: waE2E.ProtocolMessage_MESSAGE_EDIT.Enum(),
			Key: &waCommon.MessageKey{
			...
			TimestampMS: proto.Int64(1718069280),
			EditedMessage: &waE2E.Message{
				Conversation: proto.String(newMsg),
			},
		},
	}
	return sendMessage(chatID, newmsg)
...
func sendSpoofedReplyMessageInPrivate(chatID types.JID, fromID types.JID, msgID string, replyText string, myTtext string) (*waE2E.Message, *whatsmeow.SendResponse, error) {
	msg := &waE2E.Message{
		ExtendedTextMessage: &waE2E.ExtendedTextMessage{
			Text: proto.String(myTtext),
			ContextInfo: &waE2E.ContextInfo{
...
func sendSpoofedReplyImg(chatID types.JID, fromID types.JID, msgID string, file string, replyText string, myTtext string) (*waE2E.Message, *whatsmeow.SendResponse, error) {
	...
	if err != nil {
		logger.Errorf("Failed to read file file=%s error=%v", file, err)
		saveSentError("Error sending spoofed reply img: Failed to read file", nil, nil, err)
	...
	if err != nil {
		...
		saveSentError("Error sending spoofed reply img: Failed to upload file", nil, nil, err)
		return &waE2E.Message{}, &whatsmeow.SendResponse{}, err
	}
	msg := &waE2E.Message{
		ExtendedTextMessage: &waE2E.ExtendedTextMessage{
...
func sendSpoofedReplyLocation(chatID types.JID, fromID types.JID, msgID string, myTtext string) (*waE2E.Message, *whatsmeow.SendResponse, error) {
	msg := &waE2E.Message{
		ExtendedTextMessage: &waE2E.ExtendedTextMessage{
			Text:        proto.String(myTtext),
...
func sendSpoofedTalkDemo(chatJID types.JID, spoofedJID types.JID, toGender string, language string, spoofedFile string) {
	...
	SendConversationMessage(chatJID, msgmap[language]["generic"][0], nil)
	SendConversationMessage(chatJID, msgmap[language]["generic"][1], nil)
	sendSpoofedReplyMessage(chatJID, spoofedJID, client.GenerateMessageID(), msgmap[language][toGender][0], msgmap[language]["generic"][2])
	SendConversationMessage(chatJID, msgmap[language]["generic"][3], nil)
	SendConversationMessage(chatJID, msgmap[language]["generic"][4], nil)
	if spoofedFile != "" {
		sendSpoofedReplyImg(chatJID, spoofedJID, client.GenerateMessageID(), spoofedFile, msgmap[language][toGender][1], msgmap[language]["generic"][5])
	}
