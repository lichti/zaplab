...
**`stCellStyle(count)`** — returns inline style string setting `--heat-light` and `--heat-dark` CSS custom properties. Five intensity levels (same green scale as GitHub):
...
- **Activity Heatmap**: CSS grid `grid-template-columns: repeat(24, 1fr)` × 7 rows; outer loop over DOW 0–6 (Sun–Sat), inner loop over hours 00–23; each cell is a `div.heat-cell` with `:style="stCellStyle(stHeatCount(dow, hour))"` and `:title="stCellTitle(dow, hour, count)"`
