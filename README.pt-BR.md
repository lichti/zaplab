...
| **Auto-Reply Rules** | Motor de resposta automática: avalia mensagens recebidas contra uma lista priorizada de regras; condições incluem direção do remetente (others/me/all), chat/remetente JID específico, padrão de texto (prefix/contains/exact/regex), sensibilidade a maiúsculas e janela horária (com suporte a virada da meia-noite); ações: `reply` (resposta citada com variáveis `{name}` `{sender}` `{chat}` `{text}` `{hour}`), `webhook` (POST assíncrono) e `script` (executar script armazenado); flag `stop_on_match`; contador de hits e timestamp da última correspondência por regra; API CRUD completa + endpoint de toggle |
| **Scheduled Messages** | Agenda envios futuros de mensagens armazenados na coleção `scheduled_messages`; worker de tick a cada minuto dispara até 10 mensagens pendentes por ciclo; ciclo de status: pending → sent / failed / cancelled; API CRUD completa com `scheduled_at` ISO-8601, suporte a reply-to e cancelamento opcional |
| **Reaction Tracking** | Reações criptografadas recebidas (`EncReactionMessage`) são descriptografadas e persistidas em `message_reactions`; consulta por ID de mensagem, chat ou remetente; endpoint de estatísticas retorna top emojis e top reatores por chat |
| **Mention Tracker** | Todo JID `@mencionado` em mensagens recebidas é extraído do `ContextInfo` e persistido em `mentions`; flag `is_bot` quando o próprio JID do bot é referenciado; endpoint de estatísticas com contagens por JID e total de menções ao bot |
| **Contact Cache** | Coleção `contact_cache` armazena nome, telefone, recado e URL do avatar por JID; atualizada de forma assíncrona após cada chamada ao `GetContactInfo()`; API: busca, atualização de contato único, populate em background a partir do store do whatsmeow, exclusão de entrada |
