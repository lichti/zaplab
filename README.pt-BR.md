...
| **Scripting** | Sandbox de automação JavaScript com motor goja; scripts persistidos com nome, descrição, timeout configurável, toggle de habilitado/desabilitado; APIs da sandbox: `console.log`, `wa.sendText(jid, text)`, `wa.status()`, `http.get(url)`, `http.post(url, body)`, `db.query(sql)`, `sleep(ms)`; console ad-hoc para execução rápida sem salvar; status/saída/duração da última execução armazenados por script |
